...
# Import Results (Context only: "I only have [MASK]")
```{r}
# Import result files 
...
sca_data = read_csv("~/laila_johnston@brown.edu - Google Drive/Shared drives/BLT Lab/Current Studies/SCA (Structure of Computational Alternatives; formerly CompAlt and RSA)/Extensions (Laila)/data/sca_dataframe_filtered.csv",
                    col_types = cols(
                      story             = col_character(),
...
```{r}
# Define the tile plot function
make_tile_plot <- function(df, title = NULL) {
  # Use fixed scale for Empirical Probability and Human Negation
  ...
  ggplot(df, aes(x = cleaned_trigger, y = cleaned_query, fill = value)) +
    ...
    labs(title = title, x = "Trigger", y = "Query") +
    theme(
...
for (ctx in contexts) {
  ...
  plot_order <- c(
    "Set Empirical Probability", "Set Log Likelihood", 
    "Ordering Empirical Probability", "Ordering Log Likelihood", 
    "Conjunction Empirical Probability", "Conjunction Log Likelihood", 
    "Disjunction Empirical Probability", "Disjunction Log Likelihood", 
    "Always Negate Empirical Probability", "Always Negate Log Likelihood", 
    "Never Negate Empirical Probability", "Never Negate Log Likelihood", 
    "Human Negation"
  )
...
# after you’ve built p
# ggsave(
#   filename = "~/laila_johnston@brown.edu - Google Drive/Shared drives/BLT Lab/Current Studies/SCA (Structure of Computational Alternatives; formerly CompAlt and RSA)/Extensions (Laila)/figures/log_likelihood_set.png",
#   plot     = p,
#   width    = 12,    
#   height   = 8,
#   dpi      = 300,
